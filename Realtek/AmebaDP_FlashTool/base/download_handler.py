...
class Ameba(object):
    ...
    def read_flash(self, output):
        self.logger.info(f"Flash capacity is {self.device_info.flash_capacity}")
        with open(output, 'wb') as stream:
         addr = 0x08000000;
         while(addr < 0x08000000 + self.device_info.flash_capacity):
             self.logger.info(f"Reading at {addr:X}")
             ret, resp = self.floader_handler.read(MemoryInfo.MEMORY_TYPE_NOR, addr, 0x400);
             addr += 0x400
             stream.write(resp[:-1])
